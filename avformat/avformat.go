...
const (
	...
	AVFMT_FLAG_PRIV_OPT        = 0x20000  ///< Enable use of private options by delaying codec open (deprecated, does nothing)
...
func (ctxt *Context) AvIOClose() int {
	pb := ((*C.struct_AVFormatContext)(ctxt)).pb
	return int(C.avio_closep(&pb))
}

func AvIOOpenNew(res *AvIOContext, url string, flags int) (err error) {
	urlStr := C.CString(url)
	defer C.free(unsafe.Pointer(urlStr))
	...
	//defer C.free(unsafe.Pointer(urlStr))
	//// err = avutil.ErrorFromCode(int(C.avio_open((**C.AVIOContext)(unsafe.Pointer(&res)), urlStr, C.int(flags))))
	//// fmt.Printf("AvIOOpenNew error is %v\n", err)
	//fmt.Printf("[avformat go 1] %p org:%p now:%p\n", *res, res, (**C.AVIOContext)(unsafe.Pointer(res)))
	//err = avutil.ErrorFromCode(int(C.avio_open((**C.AVIOContext)(unsafe.Pointer(res)), urlStr,C.int(flags))))
	//fmt.Printf("[avformat go 2] %p org:%p now:%p\n", *res, res, (**C.AVIOContext)(unsafe.Pointer(res)))
	//return err
}

func AvOptSetInit(ctxt *Context, name string, val int64, searchFlags int) int {
	return int(C.av_opt_set_int(((*C.struct_AVFormatContext)(ctxt)).priv_data, C.CString(name), C.int64_t(val), C.int(searchFlags)))
}
func AvOptSet1(ctxt *Context, name string, val string, searchFlags int) int {
	return int(C.av_opt_set(((*C.struct_AVFormatContext)(ctxt)).priv_data, C.CString(name), C.CString(val), C.int(searchFlags)))
}

func (context *Context) AvSetFormat(f *OutputFormat) {
	context.oformat = (*C.struct_AVOutputFormat)(f)
}

func (context *Context) AvSetUrl(url string) {
	context.url = C.CString(url)
}
