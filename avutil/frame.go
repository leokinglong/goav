...
const (
	AV_PICTURE_TYPE_NONE = int(C.AV_PICTURE_TYPE_NONE) ///< Undefined
	AV_PICTURE_TYPE_I    = int(C.AV_PICTURE_TYPE_I)    ///< Intra
	AV_PICTURE_TYPE_P    = int(C.AV_PICTURE_TYPE_P)    ///< Predicted
	AV_PICTURE_TYPE_B    = int(C.AV_PICTURE_TYPE_B)    ///< Bi-dir predicted
	AV_PICTURE_TYPE_S    = int(C.AV_PICTURE_TYPE_S)    ///< S(GMC)-VOP MPEG-4
	AV_PICTURE_TYPE_SI   = int(C.AV_PICTURE_TYPE_SI)   ///< Switching Intra
	AV_PICTURE_TYPE_SP   = int(C.AV_PICTURE_TYPE_SP)   ///< Switching Predicted
	AV_PICTURE_TYPE_BI   = int(C.AV_PICTURE_TYPE_BI)   ///< BI type
)
...
func AvFrameCopy(d, s *Frame) int {
	return int(C.av_frame_copy((*C.struct_AVFrame)(unsafe.Pointer(d)), (*C.struct_AVFrame)(unsafe.Pointer(s))))
}
...
//GetPicture creates a YUV420 from the frame
func GetPictureYUV420(frame *Frame, file *os.File) {
	w := int(frame.linesize[0])
	h := int(frame.height)
	fmt.Printf("w is %d, h is %d \n", w, h)
	//Y
	if unsafe.Pointer(frame.data[0]) != nil {
		file.Write(C.GoBytes(unsafe.Pointer(frame.data[0]), C.int(w*h)))
	}
	//U
	if unsafe.Pointer(frame.data[1]) != nil {
		file.Write(C.GoBytes(unsafe.Pointer(frame.data[1]), C.int(w*h/4)))
	}
	//V
	if unsafe.Pointer(frame.data[2]) != nil {
		file.Write(C.GoBytes(unsafe.Pointer(frame.data[2]), C.int(w*h/4)))
	}
	return
}
...
func AvFrameGetNbSamples(f *Frame) int {
	return int(f.nb_samples)
}

func (f *Frame) AvFrameSetNbSamples(nbSamples int) {
	f.nb_samples = C.int(nbSamples)
}

func (f *Frame) AvFrameSetChannelLayout(channelLayout uint64) {
	f.channel_layout = (C.uint64_t)(channelLayout)
}

func (f *Frame) AvFrameSetFormat(format int) {
...
func AvFrameGetChannels(f *Frame) int {
	return int(f.channels)
}

func AvFrameGetLineSize(f *Frame, i int) int {
	return int(f.linesize[i])
}

func AvFrameGetFormat(f *Frame) int {
	return int(f.format)
}

//func WriteAAC(decodedFrame *Frame, file *os.File, decCtx *avcodec.Context) {
//
//	fmt.Printf("[WriteAAC] Start\n")
//	fmt.Printf("[WriteAAC] decCtx.AvCodecGetSampleFmt() %d\n", decCtx.AvCodecGetSampleFmt())
//
//	//dataSize := 8
//	dataSize := avcodec.AvGetBytesPerSample(decCtx.AvCodecGetSampleFmt())
//	fmt.Printf("[WriteAAC] dataSize is %d\n", dataSize)
//	if dataSize < 0 {
//		/* This should not occur, checking just for paranoia */
//		fmt.Printf("[ProcessOneAudioFrame] AvGetBytesPerSample is less than 0 %d\n", dataSize)
//		os.Exit(1)
//	}
//
//	fmt.Printf("[WriteAAC] AvFrameGetNbSamples(decodedFrame) %d, decCtx.Channels() %d\n", AvFrameGetNbSamples(decodedFrame), decCtx.Channels())
//	for i := 0; i < AvFrameGetNbSamples(decodedFrame); i++ {
//		for ch := 0; ch < decCtx.Channels(); ch++ {
//			file.Write(C.GoBytes(unsafe.Pointer(uintptr(unsafe.Pointer(decodedFrame.data[ch]))+uintptr(dataSize*i)), C.int(dataSize)))
//		}
//	}
//
//	return
...
func (f *Frame) SetPktDuration(duration int64) {
	f.pkt_duration = C.int64_t(duration)
}
...
func (f *Frame) AvSetHeight(height int) {
	f.height = C.int(height)
}

func (f *Frame) AvGetWidth() int {
	return int(f.width)
}

func (f *Frame) AvGetHeight() int {
	return int(f.height)
}

func (f *Frame) AvFrameGetLineSize() *unsafe.Pointer {
